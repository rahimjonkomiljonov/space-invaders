...
class BulletManager(turtle.Turtle):
    ...
    def create_bullet(self, x, y, is_player_bullet=False, angle=90):
        ...
        bullet.penup()
        bullet.goto(x, y)
        bullet.setheading(angle)  # Set bullet direction (always 90 for player bullets)
        if is_player_bullet:
    ...
    def move_bullets(self):
        ...
        for bullet in self.player_bullets[:]:
            x, y = bullet.position()
            y += self.bullet_speed  # Move straight up
            bullet.goto(x, y)
            if y > 300:  # Remove bullets that go off-screen
                bullet.hideturtle()
    ...
    def clear_bullets(self):
        ...
        self.alien_bullets.clear() #import turtle
